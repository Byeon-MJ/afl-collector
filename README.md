...
2. **"Flash 캡처 시작"** 버튼 탭
3. 화면이 순서대로 색상을 표시하며 프레임을 캡처
4. AE/AWB 분석 모드면 동일 시퀀스를 한 번 더 실행 (AE 해제 상태)
5. 세션 완료 → 다음 세션 버튼 탭 반복
...
```
afl_data_live_2026-03-13.zip
└── live/
    └── session_20260313_0001/
        ├── metadata.json           # 챌린지 정보 + 프레임별 밝기
        ├── baseline.png            # Pass 1: 검정 (AE locked)
        ├── flash_001.png           # Pass 1: Red flash
        ├── flash_002.png           # Pass 1: Green flash
        ├── flash_003.png           # Pass 1: Blue flash
        ├── flash_004.png           # Pass 1: Yellow flash
        ├── calibration.png         # Pass 1: White
        ├── ae_auto_baseline.png    # Pass 2: 검정 (AE auto) ← AE 분석 모드 전용
        ├── ae_auto_flash_001.png   # Pass 2: Red flash
        ├── ae_auto_flash_002.png   # Pass 2: Green flash
        ├── ...
        └── ae_auto_calibration.png # Pass 2: White
```
...
| 화면 flash가 안 보임 | 화면 밝기 낮음 | 밝기 최대로 설정 |
